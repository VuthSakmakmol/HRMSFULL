<template>
  <div>
    <strong>Direct Labor Chart</strong>
    <apexchart
      v-if="series.length"
      type="line"
      height="450"
...
onMounted(async () => {
  ...
  const direct: Category = data.categories.find((c: Category) => c.key === 'direct')
  const months = data.months.map(formatMonth)

  // compute fill‐rate %
  const fillRate = direct.targetBudget.map((b, i) =>
    b > 0 ? Math.round(direct.actual[i] / b * 100) : 0
  )

  // flip sign on variances so that positive means over-budget, negative under-budget
  const varianceBudget  = direct.varianceBudget.map(v => -v)
  const varianceRoadmap = direct.varianceRoadmap.map(v => -v)

  // determine y-axis minimum to accomodate negative values
  const allVars = [...varianceBudget, ...varianceRoadmap]
  const minVar = Math.min(0, ...allVars)
  const yMin = Math.floor(minVar * 1.2)

  series.value = [
    { name: 'Target Budget',   type: 'column', data: direct.targetBudget   },
    { name: 'Target Roadmap',  type: 'column', data: direct.targetRoadmap  },
    { name: 'Actual',          type: 'column', data: direct.actual         },
